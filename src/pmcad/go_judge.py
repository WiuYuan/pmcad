...
def build_go_selection_prompt(query_name: str, query_desc: str, hits: list) -> str:
    """
    要求 LLM 从 hits 中选出 “语义上最正确（最符合 query_name/query_desc 的 GO）” 的那个。

    LLM 输出格式要求严格：必须输出一个 GO 名称（与 hits 中收到的 name 完全一致）
    或输出 "None" 表示没有一个是正确的。
    """
...
Below is a QUERY GO TERM and its DESCRIPTION, followed by CANDIDATE GO TERMS.

Your task:
- Select the **single most relevant GO term** from the candidates with respect to the query.
- You MUST select one candidate if any are provided.
- Only output "None" if the candidate list is empty.

SELECTION CRITERIA:
- Choose the GO term that is most closely related in biological meaning to the query.
- Exact semantic equivalence is NOT required.
- Functional, process-level, or component-level relevance is acceptable.
- If multiple candidates are plausible, choose the most specific one.

OUTPUT FORMAT:
- ONLY output EXACTLY ONE STRING:
    - either a GO ID (for example: GO:0006413), which MUST be one of the candidate IDs listed below,
...
def match_llm_output_to_hit(llm_output: str, hits: list):
    """
    将 LLM 输出与 hits 中的 GO ID 或 name 做匹配。

    现在允许两种合法输出：
      1. "GO:0006413" 这种 GO_ID（推荐）
    ...
    for h in hits:
        go_id = h.get("id")
        if go_id and normalize(go_id) == out:
            return h
...
def process_one_folder_judge_go_id(folder: str, input_name="ds_go_uniprotapi.json", output_name="ds_go_uniprotapi.json", llm=None):
    ...
    for entry in go_list:
        ...
        # ---- 构建 prompt ----
        prompt = build_go_selection_prompt(query_name, query_desc, hits)
