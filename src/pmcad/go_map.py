...
def process_one_folder_get_go_id(
    folder: str,
    input_name: str,
    output_name: str,
    search_func,
    ...
    # 收集所有 GO（去重）
    # ---------------------------
    go_items = {}  # (name, desc) -> None

    def collect_go_from_entity(ent):
    ...
    if not go_items:
        ...
        return out, [
            {"type": "status", "name": f"pmid:{pmid} (no GO)"},
    ...
    for (name, desc) in go_items.keys():
